...
export const BookingHistoryPage = ({ onBack, bookings, t }) => {
    const { deleteBooking, setConfirmCancel, confirmCancel } = useAppContext();

    const handleConfirmDelete = () => {
        if (confirmCancel) {
            deleteBooking(confirmCancel.id);
                        ...
                        {bookings.length > 0 ? bookings.map(booking => (
                            <li key={booking.id} className="p-4 flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 dark:text-gray-400">{booking.date.toLocaleString()}</p>
                                </div>
                                {booking.status === 'Upcoming' ? (
                                    < div className="flex items-center space-x-2">
                                        <span className="px-2 py-1 text-xs font-medium rounded-full bg-blue-900/70 text-blue-300">{t(booking.status)}</span>

                                        <button onClick={() => setConfirmCancel(booking.id)} className="px-3 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800 hover:bg-red-200">Cancel</button>


                                    </div>
                                ) : (
                                    <span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">{t(booking.status)}</span>
                                )}
                            </li>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-zinc-800 p-6 rounded-2xl shadow-xl text-center w-80">
                        ...
                        <div className="flex space-x-2">
                            <button onClick={() => setConfirmCancel(null)} className="flex-1 py-2 bg-gray-200 dark:bg-zinc-700 rounded-lg">No</button>
                            <button onClick={() => handleConfirmDelete(confirmCancel)} className="flex-1 py-2 bg-red-500 text-white rounded-lg">Yes, Cancel</button>
                        </div>
                    </div>
                </div>
            )}
